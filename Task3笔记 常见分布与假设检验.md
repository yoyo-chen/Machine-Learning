...
简单点讲，之所以有以上种种麻烦，就是因为我们在检验问题里设定的临界值C，和假设里的$\theta_0$不是完全相等的，是更宽松的限值（例如原假设为$\theta \geq \theta_0$，那么$C<\theta_0$），所以估计值有可能在C和$\theta_0$之间，而$\theta_0$严格划分了原假设和对立假设。在$\theta_0$附近，估计值比$\theta_0$小还是比$\theta_0$大的概率差不多，在这个暧昧的区间上，想要一侧概率很小而另一侧概率很大是不可能的，即原假设为真时拒绝的概率很小而原假设为假时拒绝的概率很大是无法实现的。
